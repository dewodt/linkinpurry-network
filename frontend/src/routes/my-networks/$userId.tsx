...
import { ListConnectionErrorResponse, ListConnectionResponseBody, ListConnectionSuccessResponse } from '@/types/api/connection';
...
function RouteComponent() {
  ...
  const filteredConnections = data?.data.connections?.filter((connection) => connection.name.toLowerCase().includes(searchQuery.toLowerCase()));
    ...
    <main className="flex min-h-[calc(100vh-4rem)] flex-auto flex-col items-center gap-5 bg-muted p-6 py-12 sm:p-12">
      <div className="w-full max-w-4xl rounded-lg bg-white p-6 shadow-sm">
        <div className="mb-6 flex items-center justify-between">
          <h1 className="text-2xl font-semibold">{data?.data.connections?.length || 0} Connections</h1>
